...
class DbManager(models.Manager):
    def regValidation(self, postData, filesData):
        ...
        # TODO: add 18 years to dob_checker
        ...
        if postData['dob']>dob_checker:
            errors.append("You should be at least 18 years old.")
        ...
        if len(errors) > 0:
            ...
            response['user'] = User.objects.create(
                ...
                phonenumber=postData['phonenum'],
                dob=postData['dob'],
                photo=filesData['image']
            )
        return response
    ...
    def loginValidation(self, postData):
        ...
        if len(users) < 1:
            ...
            response['errors'] = errors
            return response
        password_check = bcrypt.checkpw(postData['logpassword'].encode(), users[0].password.encode())
        if password_check:
    ...
    def activityValidation(self, postData):
        ...
        if len(errors) == 0:
            ...
            response['activity'] = Activity.objects.create(
                ...
                endtime=endtime,
                created_by=User.objects.get(id=postData['activeUser']),
