...
def dash(request):
    ...
    if 'writeTo' in request.session:
        ...
        allmsg = Message.objects.filter(written_by=activeuser).filter(written_for=otheruser).order_by(
            'created_at') | Message.objects.filter(written_for=activeuser).filter(written_by=otheruser).order_by(
            'created_at')
        msgcheck = True
    groupedmsg = Message.objects.raw(
        'SELECT * FROM finder_app_message WHERE written_for_id ={} GROUP BY written_by_id'.format(activeuser.id))
    groupedmsg2 = Message.objects.raw(
        'SELECT * FROM finder_app_message WHERE written_by_id ={} GROUP BY written_by_id'.format(activeuser.id))
...
def processMessage(request):
    ...
    Message.objects.create(
		...
		written_by=activeuser,
		written_for=otheruser,
